...
import 'clsx';
import { $ as $$CardTitle, a as $$Image, b as $$ProjectLayoutLinks, c as $$Layout, g as getEntry } from './a3_e9861986.mjs';
/* empty css                             */
...
const $$Card = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="relative col-start-2 col-span-10 mb-28 max-sm:col-start-3 max-sm:col-span-9"><a class="mt-10 mb-10"${addAttribute(project?.data.projectURL, "href")} target="_blank">${renderComponent($$result, "CardTitle", $$CardTitle, { "project": project, "i": i })}<p class="italic">${project?.data.projectSubtitle}</p><div class="block relative card-height mt-4"><div class="absolute top-0 bottom-0 right-0 m-auto h-full w-full overflow-hidden flex justify-center"><img${addAttribute(`/images/${project?.data.thumbnail}.webp`, "src")} class="object-cover max-h-none max-w-none"></div><div class="absolute z-50 -bottom-4 -right-4 flex flex-col gap-5 font-bold py-4 w-11/12 text-right css-glass-bg px-6 max-md:bottom-4 max-md:right-4"><ul${addAttribute(``, "class")}>${project?.data.projectBulletList.map((option) => renderTemplate`<li class="font-semibold">✦ ${option}</li>`)}</ul><div class="flex justify-end">${project && renderTemplate`<div class="flex gap-2 h-fit flex-wrap justify-end">${project.data.technologies?.map(
